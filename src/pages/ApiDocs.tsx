    ...
    <div className="space-y-8 max-w-[900px]">
      ...
      <GlassCard delay={0.08} corners className="p-6 space-y-4">
        ...
        <ol className="list-decimal list-inside text-sm text-white/50 space-y-2.5 ml-1">
          ...
          <li>Install SDK: <code className="font-mono text-[11px] bg-white/[0.04] border border-white/[0.06] px-2 py-0.5 rounded-md text-white/60">npm install @agentic-darkpool/sdk</code></li>
      ...
      <GlassCard delay={0.16} gradient className="overflow-hidden">
        ...
        <pre className="p-5 text-[12px] font-mono overflow-x-auto leading-relaxed">
          ...
          <TableBody>
            {endpoints.map((ep, i) => (
              <TableRow key={i} className="border-white/[0.04] hover:bg-white/[0.02] transition-colors duration-300">
                <TableCell className="font-mono text-sm text-white/60">{ep.path}</TableCell>
                <TableCell>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-mono font-medium border ${methodColor[ep.method]}`}>
                    {ep.method}
                  </span>
                </TableCell>
                <TableCell className="text-sm text-white/40">{ep.desc}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
